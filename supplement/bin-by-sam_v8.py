...
parser.add_option("-p", "--ploidy", dest="ploidy", type = "float", default=2.0, help="Ploidy multiplier, default is 2 for diploid")
...
parser.add_option("-R", "--reflist", dest="ref", default = "NA", help="Only run on reference seqs from this list")
...
numblanks = max(max(sizes)/opt.binsize/10, 10)
...
if opt.mode == "S":
   ...
   for file in todo:
      ...
      print file
      libname = file.split("/")[-1].split('.')[0].replace('_aln','')
      ...
      for x in f:
         ...
         if l[2] == '*':
            continue
         if int(l[1]) > 300 and int(l[1]) < 2000:
   ...
   for file in todo:
      ...
      while 1:
         ...
         if rlist == 1:
         	if l1[2] not in vref or l2[2] not in vref:
         		continue
         	elif l1[2] in vref and l2[2] not in vref:
         		print("split 1")
         		continue
         	elif l1[2] not in vref and l2[2] in vref:
         		print("split 2")
         		continue
         ...
         if l1[2] != l2[2]:
         	#different chrom, HANDLE THIS LATER
         	continue
         ...
         key2 = ""
         key3 = ""
#          if libname == "Chr01-scaffold_69" and key3 > 18:
#    			x[231211232]
         if opt.mode != "PS":
            ...
            key2 = l1[2]
            if key2 not in remlist:
#                if key2 == "Chr01-scaffold_69" and key3 > 18:
#    					x[231211232]
...
control = opt.f
control = control.split("/")[-1].split('.')[0].replace('_aln','')
...
#       x[231211232]
   ...
   for modbin in bins:
      ...
      o.write('\t'.join(fline)+'\n')
   if opt.breaks == True and len(bins) != 0:
